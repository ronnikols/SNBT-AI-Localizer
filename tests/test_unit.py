...
def test_crusoe_provider_support():
    from core import get_base_url, get_provider_class, detect_provider, PROVIDER_DEFAULTS
    from unittest.mock import patch, AsyncMock

    # Test base URL
    assert get_base_url("Crusoe Cloud") == "https://api.inference.crusoecloud.com/v1"

    # Test default model
    assert PROVIDER_DEFAULTS["Crusoe Cloud"] == "zai/GLM-5.3-Flash"

    # Test key prefix detection
    assert detect_provider("cr_test_key_123") == "Crusoe Cloud"

    # Test provider class
    provider = get_provider_class("Crusoe Cloud")
    assert provider is not None
    ...
    with patch.object(provider, 'send_request', new_callable=AsyncMock) as mock_send:
        ...
        assert call_args[0][0] == ["test text"]
        assert call_args[0][1] == "test_api_key"
        assert call_args[0][2] == "zai/GLM-5.3-Flash"

def test_key_verification_question():
    from core import test_key_with_question, TEST_QUESTIONS
    ...
    mock_provider.send_request = AsyncMock(return_value=["Blue"])
    with patch('core.get_provider_class', return_value=mock_provider):
        status, answer = asyncio.run(test_key_with_question("OpenAI", "sk-test", "gpt-4o-mini", "What color is the sky?"))
    assert (status, answer) == ("Active", "Blue")
    call_args = mock_provider.send_request.call_args[0]
    assert call_args[0] == ["What color is the sky?"]
    assert call_args[1] == "sk-test"
    assert "EXACTLY ONE word" in call_args[6]

    def raise_status(code):
        def _raise(*args, **kwargs):
            resp = httpx.Response(code, request=httpx.Request("POST", "https://api.openai.com/v1/chat/completions"))
            raise httpx.HTTPStatusError(f"{code}", request=resp.request, response=resp)
        return _raise

    for code, expected in ((401, "Invalid"), (403, "Invalid"), (429, "Rate Limited"), (500, "Unreachable")):
        mock_provider.send_request = AsyncMock(side_effect=raise_status(code))
        with patch('core.get_provider_class', return_value=mock_provider):
            status, answer = asyncio.run(test_key_with_question("OpenAI", "k", "m", "q"))
        assert (status, answer) == (expected, "")

    mock_provider.send_request = AsyncMock(side_effect=ValueError("Invalid response format"))
    with patch('core.get_provider_class', return_value=mock_provider):
        status, answer = asyncio.run(test_key_with_question("OpenAI", "k", "m", "q"))
    ...
    async def slow_request(*args, **kwargs):
        ...
        return ["slow"]
    mock_provider.send_request = AsyncMock(side_effect=slow_request)
    with patch('core.get_provider_class', return_value=mock_provider):
        status, answer = asyncio.run(test_key_with_question("OpenAI", "k", "m", "q", timeout=0.1))
    assert (status, answer) == ("Timeout", "")
...
def test_find_quest_dirs_ftb(tmp_path):
    from gui import find_all_quest_dirs
    inst = tmp_path / "FtbPack"
    quests = inst / "minecraft" / "config" / "ftbquests" / "quests"
    quests.mkdir(parents=True)
    assert find_all_quest_dirs(inst) == [quests]

def test_find_quest_dirs_kubejs_lang(tmp_path):
    from gui import find_all_quest_dirs
    inst = tmp_path / "VampPack"
    lang = inst / "minecraft" / "kubejs" / "assets" / "vamp" / "lang"
    lang.mkdir(parents=True)
    (lang / "en_us.json").write_text('{"key": "Hello"}', encoding="utf-8")
    assert find_all_quest_dirs(inst) == [inst]

def test_find_quest_dirs_kubejs_no_en_us(tmp_path):
    from gui import find_all_quest_dirs
    inst = tmp_path / "Pack"
    lang = inst / "kubejs" / "assets" / "x" / "lang"
    lang.mkdir(parents=True)
    (lang / "ru_ru.json").write_text('{}', encoding="utf-8")
    assert find_all_quest_dirs(inst) == []

def test_find_quest_dirs_empty(tmp_path):
    from gui import find_all_quest_dirs
    inst = tmp_path / "EmptyPack"
    (inst / "config").mkdir(parents=True)
    assert find_all_quest_dirs(inst) == []

def test_main_find_quest_dirs_kubejs_lang(tmp_path):
    from main import find_all_quest_dirs
    inst = tmp_path / "VampPack"
    lang = inst / "minecraft" / "kubejs" / "assets" / "vamp" / "lang"
    lang.mkdir(parents=True)
    (lang / "en_us.json").write_text('{"key": "Hello"}', encoding="utf-8")
...
def test_abort_watchdog_cancels_inflight():
    import asyncio, time
    from gui import run_with_abort_watchdog
    ...
    async def scenario():
        ...
        try:
            await run_with_abort_watchdog(slow(), lambda: state["aborted"])
...
def test_abort_watchdog_no_abort():
    import asyncio
    from gui import run_with_abort_watchdog
    ...
    async def scenario():
        res = await run_with_abort_watchdog(quick(), lambda: False)
        assert res == 42

    asyncio.run(scenario())

def test_runinfra_provider_support():
    from core import get_base_url, get_provider_class, detect_provider, PROVIDER_DEFAULTS
    from unittest.mock import patch, AsyncMock

    assert get_base_url("RunInfra") == "https://api.runinfra.ai/v1"
    assert PROVIDER_DEFAULTS["RunInfra"] == "glm-5-3-flash"
    assert detect_provider("rp_test_key_123") == "RunInfra"

    provider = get_provider_class("RunInfra")
    assert provider is not None
    ...
    with patch.object(provider, 'send_request', new_callable=AsyncMock) as mock_send:
        ...
        assert result == ["translated text"]
        mock_send.assert_called_once()
        assert mock_send.call_args[0][2] == "glm-5-3-flash"

def test_runinfra_smart_parse():
    from config import ConfigManager
